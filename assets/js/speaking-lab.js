(function (window) {
    'use strict';
    // Lucide file-music icon: ../licenses/lucide-file-music.txt (ISC).
    ...
    var voiceprintRegistrationAvailable = true;
    var voiceprintRegistrationMessage = '';
    var voiceprintLoadGeneration = 0;
    ...
    function renderMyVoiceprint(result) {
        ...
        voiceprintRegistrationAvailable = !(result && result.registration && result.registration.available === false);
        voiceprintRegistrationMessage = result && result.registration && result.registration.message || '';
        ...
        if (button) button.disabled = !voiceprintProviderConfigured || !voiceprintRegistrationAvailable;
    ...
    function loadMyVoiceprint(checkCapacity) {
        var generation = ++voiceprintLoadGeneration;
        return call('getMyVoiceprint', checkCapacity ? { check_capacity: true } : {}).then(function (result) {
            if (generation !== voiceprintLoadGeneration) return null;
            renderMyVoiceprint(result);
            if (checkCapacity) document.getElementById('voiceprint-message').textContent = voiceprintRegistrationMessage || (voiceprintProviderConfigured ? 'Hold the microphone for at least 10 seconds. Release when you finish.' : 'Voiceprint registration is temporarily unavailable.');
            return result;
        }).catch(function (error) {
            if (generation !== voiceprintLoadGeneration) return;
    ...
    function resetVoiceprintPage(options) {
        ...
        var active = Boolean(voiceprintTarget && voiceprintTarget.voiceprint && voiceprintTarget.voiceprint.status === 'active');
        recordButton.disabled = !voiceprintProviderConfigured || !voiceprintRegistrationAvailable;
        ...
        if (!options || !options.keepMessage) document.getElementById('voiceprint-message').textContent = voiceprintRegistrationMessage || (voiceprintProviderConfigured ? 'Hold the microphone for at least 10 seconds. Release when you finish.' : 'Voiceprint registration is temporarily unavailable.');
    }
    ...
    function startMyVoiceprintRecording(event) {
        if (voiceprintSaving || voiceprintController || voiceprintPressActive || !voiceprintProviderConfigured || !voiceprintRegistrationAvailable) return;
    ...
    function createDiscussionFromSet(set) {
        ...
        return call('createDiscussion', {
            ...
            if (button) { button.disabled = false; button.querySelector('[data-discussion-start-label]').textContent = 'Start'; }
        });
    }
    ...
    function renderSpeakingSetDetail(set) {
        ...
        detail.innerHTML = '<article class="speaking-set-detail">' +
            '<section class="speaking-set-context speaking-report-card" data-speaking-reading-section="context"><header class="speaking-set-section-head speaking-set-section-head-centered speaking-set-section-head-with-controls speaking-set-context-head"><p class="eyebrow accent">' + esc(setIdentity || set.display_label || 'Set') + '</p><h2 class="speaking-set-context-title">' + esc(context.title || set.title || '') + '</h2>' + speakingSetTextSizeMarkup('context', 'Context') + '</header><div class="speaking-set-context-body">' + (context.body || []).map(function (paragraph) { return '<p>' + esc(paragraph) + '</p>'; }).join('') + '</div></section>' +
            '<section class="speaking-set-part speaking-set-part-a speaking-report-card" data-speaking-reading-section="part-a"><header class="speaking-set-section-head speaking-set-section-head-centered speaking-set-section-head-with-controls"><p class="eyebrow accent">PART A - GROUP DISCUSSION</p>' + speakingSetTextSizeMarkup('part-a', 'Part A') + '</header>' + (partA.task ? '<p class="speaking-set-task"><strong>Task</strong><span>' + esc(partA.task) + '</span></p>' : '') + '<p class="speaking-set-instruction">' + esc(partA.instruction || 'You may want to talk about:') + '</p><ol class="speaking-set-points">' + points + '</ol><div class="speaking-detail-actions speaking-set-primary-action"><button class="speaking-discussion-start" id="start-set-discussion" type="button"><span class="speaking-discussion-start-icon"><svg viewBox="0 0 32 32" aria-hidden="true"><rect x="11" y="4" width="10" height="16" rx="5"/><path d="M7 16a9 9 0 0 0 18 0M16 25v4M12 29h8"/></svg></span><span data-discussion-start-label>Start</span></button></div></section>' +
    ...
    function renderVoiceprintMain() {
        ...
        voiceprintRegistrationAvailable = false;
        voiceprintRegistrationMessage = 'Checking voiceprint availability…';
        resetVoiceprintPage();
        updateToolbar({ title: 'Voiceprint', invitation: true });
        loadMyVoiceprint(true);
    }
    ...
    function detailMarkup(item) {
        ...
        var recording = canRecord ? window.MrCatSpeakingRecorder.markup({ targetSeconds: item.duration_seconds, date: item.discussion_date }) : '';
    ...
    function recordingNeedsDiscardConfirmation() { return recordingState === 'review'; }
    function syncRecordingState(snapshot) {
        recordingState = snapshot.state;
        recordingBlob = snapshot.blob;
        recordingUploadOperationId = snapshot.operationId;
        var locked = recordingLocksPage();
        document.body.classList.toggle('speaking-recording-locked', locked);
        document.body.classList.toggle('speaking-recording-flow-active', recordingState !== 'idle');
        ...
        detail.querySelectorAll('.speaking-detail-body > *').forEach(function (section) {
            ...
            section.inert = recordingState !== 'idle';
            section.setAttribute('aria-disabled', recordingState !== 'idle' ? 'true' : 'false');
        });
    ...
    function setRecordingState(nextState) { if (formalRecorder) formalRecorder.setState(nextState); }
    function stopRecordingPreview() { if (formalRecorder) formalRecorder.stopPreview(); }
    function discardLocalRecording() { if (formalRecorder) formalRecorder.discard(); }
    ...
    function persistRecordingTarget(seconds) {
        var durationSeconds = seconds || (formalRecorder && formalRecorder.target()) || 480;
        call('updateDiscussionDuration', { discussion_id: selectedId, duration_seconds: durationSeconds }).catch(function (error) {
            setStatus('The target length could not be saved. ' + friendlyError(error), true);
        });
    ...
    function cancelResponseCues() {
        responseCueNodes.forEach(function (cue) { try { cue.oscillator.stop(); } catch (_error) {} cue.oscillator.disconnect(); cue.gain.disconnect(); });
        responseCueNodes = [];
    }
    function prepareResponseCueAudio() {
        var AudioCtor = window.AudioContext || window.webkitAudioContext;
        if (!AudioCtor) return;
        try {
            if (!responseCueContext || responseCueContext.state === 'closed') responseCueContext = new AudioCtor();
            if (responseCueContext.state === 'suspended') responseCueContext.resume().catch(function () {});
        } catch (_error) { responseCueContext = null; }
    }
    function scheduleResponseCues(seconds, generation) {
        if (!responseCueContext || responseCueContext.state !== 'running') return;
        var baseTime = responseCueContext.currentTime;
        seconds.forEach(function (second, index) {
            if (generation !== responseCaptureGeneration) return;
            var last = index === seconds.length - 1;
            var length = last ? 0.4 : 0.18;
            try {
                var oscillator = responseCueContext.createOscillator(), gain = responseCueContext.createGain();
                var at = baseTime + second;
                oscillator.type = 'sine'; oscillator.frequency.setValueAtTime(last ? 1320 : 880, at);
                gain.gain.setValueAtTime(0, at); gain.gain.linearRampToValueAtTime(0.14, at + 0.012); gain.gain.exponentialRampToValueAtTime(0.001, at + length - 0.02);
                oscillator.connect(gain); gain.connect(responseCueContext.destination);
                oscillator.start(at); oscillator.stop(at + length);
                responseCueNodes.push({ oscillator: oscillator, gain: gain });
    ...
    function stopResponseHardware() {
        ...
        if (responseDeadline) window.clearTimeout(responseDeadline);
        responseTimer = 0; responseDeadline = 0;
        cancelResponseCues();
        var device = responseRecorder; responseRecorder = null;
        if (device) { device.ondataavailable = null; device.onstop = null; device.onerror = null; if (device.state !== 'inactive') { try { device.stop(); } catch (_error) {} } }
        var input = responseStream; responseStream = null;
    ...
    function renderIndividualResponseDomain(key, label, report) {
        ...
        function points(items, weakness) {
            if (!Array.isArray(items) && weakness && domain.commentary_zh) return '<p class="speaking-ir-feedback-empty">舊版評語</p><p class="speaking-ir-feedback-empty">' + esc(domain.commentary_zh) + '</p>';
            if (!Array.isArray(items) || !items.length) return '<p class="speaking-ir-feedback-empty">' + (Array.isArray(items) ? '本次可用證據不足，未列出可可靠確認的' + (weakness ? '改進建議' : '強項') + '。' : '此舊版報告未包含分項' + (weakness ? '改進建議' : '強項') + '。') + '</p>';
    ...
    function responseDialogRecorderMarkup() {
        return '<div class="speaking-response-dialog-recorder" id="response-recorder" data-state="idle"><div class="speaking-response-dial"><svg class="speaking-response-ring" viewBox="0 0 222 222" aria-hidden="true"><circle class="speaking-response-ring-track" cx="111" cy="111" r="106"/><circle class="speaking-response-ring-progress" id="response-ring-progress" cx="111" cy="111" r="106" pathLength="1"/></svg><button class="speaking-response-microphone" type="button" id="response-record" aria-label="Tap to Record"><svg class="speaking-response-mic-icon" viewBox="0 0 32 32" aria-hidden="true"><rect x="11" y="4" width="10" height="16" rx="5"/><path d="M7.5 16a8.5 8.5 0 0 0 17 0M16 24.5V28M12 28h8"/></svg><span class="speaking-response-recording-indicator" id="response-recording-indicator" hidden><span class="speaking-response-recording-dot" aria-hidden="true"></span><span>Recording</span></span><svg class="speaking-response-finished-icon" viewBox="0 0 32 32" aria-hidden="true"><path d="m7 16 6 6L25 10"/></svg><span class="speaking-response-opening-digit" id="response-opening-digit" aria-live="polite">3</span><span class="speaking-response-microphone-label" data-response-record-label>Tap to Record</span></button></div><div class="speaking-response-clock" id="response-timer" role="timer" aria-label="Time left">01:00</div><div class="speaking-response-footer"><label class="speaking-response-dialog-file" id="response-file-label"><svg viewBox="0 0 24 24" aria-hidden="true" data-icon="file-music"><path d="M11.65 22H18a2 2 0 0 0 2-2V8a2.4 2.4 0 0 0-.706-1.706l-3.588-3.588A2.4 2.4 0 0 0 14 2H6a2 2 0 0 0-2 2v10.35"/><path d="M14 2v5a1 1 0 0 0 1 1h5"/><path d="M8 20v-7l3 1.474"/><circle cx="6" cy="20" r="2"/></svg>Upload Files<input type="file" id="response-file" accept="audio/*" class="speaking-response-file-input" aria-label="Upload Files"></label><button class="primary-button speaking-response-dialog-upload" type="button" id="response-upload" disabled hidden>Submit</button></div><p class="speaking-response-status" id="response-status" role="status" aria-live="polite"></p></div>';
    }
    ...
    function closeIndividualResponseDialog() {
        ...
        var recording = responseCaptureActive();
    ...
    function bindIndividualResponseRecording(response) {
        ...
        var digit = document.getElementById('response-opening-digit');
        var fileLabel = document.getElementById('response-file-label');
        ...
        function setRecordButton(label, state) {
            ...
            record.setAttribute('aria-label', state === 'recording' || state === 'ending' ? 'Recording — Tap to stop' : label);
        ...
        function recordingFailure(message) {
            stopResponseHardware();
        ...
        if (record) record.addEventListener('click', function () {
            ...
            navigator.mediaDevices.getUserMedia({ audio: true }).then(function (stream) {
                ...
                var device = new MediaRecorder(stream, preferred ? { mimeType: preferred } : undefined);
                responseRecorder = device;
                device.ondataavailable = function (event) { if (generation === responseCaptureGeneration && event.data && event.data.size) responseChunks.push(event.data); };
                ...
                device.onstop = function () {
                    ...
                    timer.textContent = '00:00'; ring.style.strokeDashoffset = '1';
                    readyToSubmit();
                };
                stream.getTracks().forEach(function (track) { track.addEventListener('ended', function () {
                    if (generation !== responseCaptureGeneration) return;
                    if (responseCaptureState === 'recording') finishResponseRecording();
                ...
                function tick() {
                    if (generation !== responseCaptureGeneration) return;
                    if (responseCaptureState === 'countdown') {
                ...
                recordingFailure(denied ? 'Microphone access was denied. Use Upload Files instead.' : 'The microphone could not start. Please try again or use Upload Files.');
            });
        });
        if (file) file.addEventListener('change', function () {
            if (responseCaptureActive() || responseUploadInProgress) return;
            var chosen = file.files && file.files[0]; file.value = '';
            if (!chosen || (chosen.type && !/^audio\//i.test(chosen.type))) return;
            var generation = responseCaptureGeneration;
            var objectUrl = URL.createObjectURL(chosen), probe = document.createElement('audio');
            file.disabled = true; record.disabled = true; probe.preload = 'metadata';
            probe.onloadedmetadata = function () {
                URL.revokeObjectURL(objectUrl);
                if (generation !== responseCaptureGeneration || !record.isConnected) return;
                file.disabled = false; record.disabled = false;
        ...
        if (upload) upload.addEventListener('click', function () {
            if (!responseBlob || responseUploadInProgress || responseCaptureActive()) return;
            record.disabled = true; file.disabled = true;
    ...
    function startVoiceReferenceRecording(participantId, participantName, button) {
        if (recordingLocksPage()) { setStatus('Finish the formal Discussion recording first.', true); return; }
    ...
    function bindRecording() {
        if (formalRecorder) formalRecorder.destroy();
        formalRecorder = null;
        var card = detail.querySelector('.speaking-recording-card');
        if (card) formalRecorder = window.MrCatSpeakingRecorder.create(card, {
            onStateChange: syncRecordingState,
            onDateChange: persistDiscussionDate,
            onTargetChange: persistRecordingTarget,
            onUpload: uploadPreparedRecording,
            canStart: function () { return !(voiceRecorder && voiceRecorder.state !== 'inactive'); }
        });
