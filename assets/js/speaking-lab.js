(function (window) {
    ...
    function renderSpeakingSetDetail(set) {
        ...
        detail.innerHTML = '<article class="speaking-set-detail">' +
            '<section class="speaking-set-context speaking-report-card" data-speaking-reading-section="context"><header class="speaking-set-section-head speaking-set-section-head-centered speaking-set-section-head-with-controls speaking-set-context-head"><p class="eyebrow accent">' + esc(setIdentity || set.display_label || 'Set') + '</p>' + speakingSetTextSizeMarkup('context', 'Context') + '</header><h2 class="speaking-set-context-title">' + esc(context.title || set.title || '') + '</h2><div class="speaking-set-context-body">' + (context.body || []).map(function (paragraph) { return '<p>' + esc(paragraph) + '</p>'; }).join('') + '</div></section>' +
