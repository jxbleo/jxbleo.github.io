(function (window) {
    ...
    function liveMarkup() {
        ...
        return '<dialog class="speaking-recording-state speaking-recording-live" id="recording-live" tabindex="-1" hidden aria-label="Discussion recording">' +
            ...
            '<div class="speaking-recording-live-label"><span aria-hidden="true"></span><strong id="recording-live-status">Ready</strong></div>' +
    ...
    function markup(options) {
        ...
        var recording = canRecord ? '<section class="speaking-section-card speaking-recording-card" data-recording-state="idle"><header><div><h3>Record the Discussion</h3><p>Record here or choose one audio file. Nothing is uploaded until you confirm.</p></div><span class="speaking-pill" id="recording-target-pill">Target ' + esc(targetMinutes % 1 ? targetMinutes.toFixed(1) : targetMinutes) + ' min</span></header>' +
            '<div class="speaking-recording-state" id="recording-ready"><input id="recording-duration" type="hidden" value="' + esc(targetMinutes) + '"><div class="speaking-recording-choice"><button class="primary-button" type="button" id="record-now">Record on this device</button><label class="outline-button speaking-file-button" id="audio-file-label">Choose audio file<input type="file" accept="audio/*" hidden id="audio-file"></label><label class="speaking-audio-date"><span>Audio date</span><input id="recording-date" type="date" value="' + esc(item.discussion_date || shanghaiToday()) + '"></label></div><p class="speaking-recording-note">Device recordings use today. For an existing audio file, choose the date it was recorded.</p><p class="speaking-quality-warning" id="recording-message" role="status" aria-live="polite"></p></div>' +
            liveMarkup() +
    ...
    function create(root, options) {
        ...
        function setState(next, copy) {
            ...
            node('recording-live').hidden = ['ready', 'requesting', 'countdown', 'recording', 'ending', 'stopping'].indexOf(next) < 0;
            ...
            node('recording-back').hidden = ['ready', 'requesting', 'countdown'].indexOf(next) < 0;
            ...
            node('recording-live-status').textContent = { ready: 'Ready', requesting: 'Starting microphone', countdown: '', recording: 'Recording', ending: '', stopping: 'Finishing recording' }[next] || 'Recording';
            ...
            node('stop-recording').setAttribute('aria-label', next === 'ready' ? 'Start three-second countdown' : next === 'countdown' ? 'Cancel countdown' : 'Finish discussion recording');
            node('stop-recording').disabled = next === 'stopping' || next === 'requesting';
        ...
        function paintOuterWave(samples, rms, reduced) {
            var energy = Math.min(1, rms * 8), points = [];
            for (var band = 0; band < 40; band += 1) {
                var at = Math.floor((band + 0.5) * samples.length / 40);
                var input = Math.min(1, Math.abs(samples[at]) * 5.5 + energy * 0.5);
                waveLevels[band] += (input - waveLevels[band]) * 0.24;
            }
            for (var index = 0; index < 180; index += 1) {
                ...
                var radius = reduced ? 102 : 101.2 + level * 9 + energy * 1.5;
        ...
        function monitorQuality(captureGeneration) {
            ...
            try {
                ...
                function draw(now) {
                    ...
                    if (state === 'recording' && now - lastWaveAt >= 40) { paintOuterWave(samples, rms, reduced); lastWaveAt = now; }
        ...
        function startRecording() {
            ...
            if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia || !window.MediaRecorder) { warning('Recording is unavailable here. Go Back to choose an audio file.'); return; }
