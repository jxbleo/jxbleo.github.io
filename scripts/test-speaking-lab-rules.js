...
function run() {
  ...
  const reportTurns = lab.canonicalSpeakingTurns(segments, ["spk_01", "spk_02"]);
  const chunks = lab.speakingTurnReviewChunks(reportTurns, ["spk_01", "spk_02"], 8);
  assert.deepEqual(chunks.map((chunk) => chunk.chunk_id), ["spk_01_chunk_01", "spk_02_chunk_01"]);
  assert(chunks.every((chunk) => chunk.target_turns.length === 1));
  const manyTurns = Array.from({ length: 19 }, (_, index) => ({ turn_id: `${index % 2 ? "spk_02" : "spk_01"}_turn_${String(Math.floor(index / 2) + 1).padStart(2, "0")}`, speaker_key: index % 2 ? "spk_02" : "spk_01" }));
  const boundedChunks = lab.speakingTurnReviewChunks(manyTurns, ["spk_01", "spk_02"], 8);
  assert(boundedChunks.every((chunk) => chunk.target_turns.length <= 8), "no model chunk may request more than eight detailed turn reviews");
  assert.deepEqual(boundedChunks.flatMap((chunk) => chunk.target_turns.map((turn) => turn.turn_id)).sort(), manyTurns.map((turn) => turn.turn_id).sort(), "chunking must neither sample nor duplicate speaking turns");
  const overview = lab.canonicalizeReport(valid, ["spk_01", "spk_02"], segments, { candidateSpeakerKeys: ["spk_01", "spk_02"], requireTurnReviews: false });
  assert(overview.candidates.every((candidate) => candidate.turn_reviews.length === 0));
  const chunkReviews = Object.fromEntries(valid.candidates.map((candidate) => [candidate.speaker_key, lab.canonicalizeTurnReviewChunk({ speaker_key: candidate.speaker_key, turn_reviews: candidate.turn_reviews }, candidate.speaker_key, reportTurns.filter((turn) => turn.speaker_key === candidate.speaker_key))]));
  const merged = lab.mergeChunkedSpeakingReport(overview, chunkReviews, ["spk_01", "spk_02"], segments, { candidateSpeakerKeys: ["spk_01", "spk_02"] });
  assert.deepEqual(merged.candidates.map((candidate) => candidate.turn_reviews.length), [1, 1], "chunk merge must retain every speaking turn review");
  assert.throws(() => lab.canonicalizeTurnReviewChunk({ speaker_key: "spk_01", turn_reviews: [] }, "spk_01", chunks[0].target_turns), /TURN_REVIEW_COUNT/);
