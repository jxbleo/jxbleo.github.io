...
function run() {
  ...
  const recorder = read("assets/js/speaking-recorder.js");
  ...
  assert.match(teacherPage, /speaking-lab\.css\?v=20260913-ir-focus-fade-1/);
  ...
  assert.strictEqual((selectedSetMarkup.match(/speakingSetTextSizeMarkup\(/g) || []).length, 3, "Context, Part A, and Part B must each receive text-size controls");
  assert.match(selectedSetMarkup, /data-speaking-reading-section="part-b"[\s\S]*speakingSetTextSizeMarkup\('part-b', 'Part B'\)/);
  const setListMarkupStart = app.indexOf('function renderSpeakingSetResults()');
  const setListMarkup = app.slice(setListMarkupStart, app.indexOf('function renderSpeakingSetCards(items)', setListMarkupStart));
  assert.doesNotMatch(setListMarkup, /speaking-set-card-route|>Context<|>Part A<|>Part B</, "Set library cards must omit the three route labels");
  assert.doesNotMatch(setListMarkup, /DSE Paper 4/, "Set library card metadata must not repeat DSE Paper 4");
  ...
  assert.match(app, /setRecordButton\(stoppedEarly \? 'Tap to Start Over' : 'Finished', stoppedEarly \? 'stopped' : 'finished'\)/);
  assert.match(app, /setRecordButton\('Tap to Stop', 'recording'\)/);
  assert.match(app, /scheduleResponseCues\(\[0, 1, 2\]/);
  assert.match(app, /scheduleResponseCues\(\[60, 61, 62, 63, 64\]/);
  assert.match(app, /responseDeadline = window.setTimeout\(finishResponseRecording, 65000\)/);
  assert.match(app, />Submit<\/button>/);
  assert.match(app, /Finished. Record again/);
  ...
  assert.match(teacherPage, /id="teacher-speaking-recorder-dialog"[^>]*data-teacher-modal[^>]*role="dialog"[^>]*aria-modal="true"[^>]*hidden/);
  assert.match(teacher, /teacherAudioOperationId !== snapshot.operationId[\s\S]*draftUploadOperationId = ''/);
  assert.match(teacher, /discussion_date: recording.date[\s\S]*duration_seconds: recording.targetSeconds/);
  ...
  assert.match(teacher, /call\('startAudioUpload'/);
  assert.match(teacher, /uploadCloudFile\(result\.upload\.cloud_path, blob\)/);
  assert.match(teacher, /call\('finishAudioUpload'/);
  assert.match(teacher, /call\('startAnalysis'/);
  assert.match(teacher, /FILE_UPLOAD_TIMEOUT_MS = 10 \* 60 \* 1000/);
  assert.match(teacher, /function showResults\(\)/);
  assert.match(teacher, /captureState === 'analysis_retry'\) \{ recorderDialog.close\(\); showResults\(\); return; \}/, "leaving the retry state for reports must preserve the no-reupload retry path");
  assert.doesNotMatch(teacher, /editorRowMarkup|teacherUpdateSpeakingSet|teacherCreateSpeakingSet|teacherDeleteSpeakingSet/);
  ...
  assert.match(app, /onDateChange: persistDiscussionDate/);
  ...
  assert.match(css, /speaking-recording-dial[^}]*--diameter:min\(86vw,74dvh,840px\)/);
  ...
  assert.match(preUploadDetailMarkup, /MrCatSpeakingRecorder\.markup/);
  assert.match(recorder, /speaking-recording-card/);
  ...
  assert.match(page, /speaking-lab\.css\?v=20260913-ir-focus-fade-1/);
  assert.match(page, /speaking-lab\.js\?v=20260913-ir-focus-fade-1/);
