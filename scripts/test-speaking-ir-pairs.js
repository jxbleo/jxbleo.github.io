...
articles.forEach(([,attrs,body],i)=>{assert.equal((body.match(/<section /g)||[]).length,2);assert(body.indexOf('Thinking prompts')<body.indexOf('Sample'));assert(body.includes(input.sample_responses[i].thinking_prompt_zh));assert.equal(/ hidden/.test(attrs),i!==0);});
...
const panels=[0,1,2].map(i=>({hidden:i!==0,getAttribute:()=>String(i)}));
