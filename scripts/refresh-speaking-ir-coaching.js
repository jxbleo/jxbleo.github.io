...
function command(table, type, data) {
  ...
  if (result.status !== 0) throw new Error(`CloudBase ${type} failed (${result.status}); no private output printed`);
...
function stamp() { return { $date: new Date().toISOString() }; }
...
function status(manifest, retry) {
...
function main(argv) {
  ...
  if (mode === "status" || mode === "retry") {
    const records = status(manifest, mode === "retry");
...
if (require.main === module) { try { main(process.argv.slice(2)); } catch (error) { console.error(error.code === "ERR_ASSERTION" ? "Safety assertion failed; inspect the private manifest locally (no student content printed)." : error.message); process.exitCode = 1; } }
