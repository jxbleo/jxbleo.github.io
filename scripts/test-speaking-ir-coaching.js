...
assert.match(compactHtml, /Your answer<\/strong><span class="speaking-ir-answer-chevron"/);
assert.match(compactHtml, /<p class="speaking-ir-answer-word-count">10 words<\/p>/);
const answerMarkup = compactHtml.match(/<details[^>]*id="response-answer"[\s\S]*?<\/details>/)[0];
assert.doesNotMatch(answerMarkup.split('</summary>')[0], /word-count|words/);
assert(answerMarkup.indexOf('10 words') > answerMarkup.indexOf("They're useful for hands-on learning."));
assert.match(compactHtml, /I think school gardens help\. They're useful for hands-on learning\./);
assert.equal((compactHtml.match(/class="speaking-ir-transcription-text"/g) || []).length, 1);
...
assert.doesNotMatch(context.renderIndividualResponseReport({ report: { transcript: [{ text: '<img src=x>' }] }, set_snapshot: { title: '<script>' } }), /<img|<script>/);
// Completed workspaces must not retain either of the old header cards.
