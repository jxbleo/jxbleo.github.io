...
async function main() {
  ...
  assert(dashboardHtml.includes('id="student-words-preview-add-trigger"'), "the preview must expose its add action in the top-left corner");
  ...
  assert(dashboardHtml.includes('<span>See All</span>'), "the preview must label its top-right workspace route See All");
  assert(!dashboardHtml.includes('student-words-dialog-footer'), "the recent-word list must use the space formerly reserved for a fixed footer");
  ...
  assert(dashboardHtml.includes('id="student-words-manual-title">Add to My Words</p>'), "manual entry must use the Change Password title hierarchy");
  assert(dashboardHtml.includes('class="primary-button" type="submit">Confirm</button>'), "manual entry must use a concise Confirm action");
  assert(!dashboardHtml.includes('placeholder="Enter a word or short phrase"') && !dashboardHtml.includes('placeholder="Enter the sentence containing this word"'), "manual inputs must remain visually empty until the student types");
  ...
  assert(dashboardJs.includes("activeWords.slice(0, 9)"), "the Dashboard preview must use the freed footer space for up to nine recent active words");
  ...
  assert(dashboardJs.includes("wordsPreviewAddMenu.focus({ preventScroll: true })") && !dashboardJs.includes("if (open && wordsPreviewAddInput) wordsPreviewAddInput.focus()"), "opening manual entry must focus the dialog instead of summoning the software keyboard");
  ...
  assert(dashboardCss.includes(".student-words-manual-title-row") && dashboardCss.includes(".dialog-actions.student-words-manual-actions"), "manual entry must mirror the Change Password title and action geometry");
