...
| 内页登录丢失当前视图、练习 query/hash，或登录后回到错误页面 | `login-navigation.js` 未在页面专属脚本前加载、cache query 版本不一致、调用没有传当前完整 URL，或 return 目标被错误地当成身份 | 检查 Dashboard / My Words / Attempt Review 的脚本顺序与 `MrCatLoginNavigation.loginHref(window.location.href, fallback)`；验证同源 `.html` 目标、外部目标拒绝、旧 `user` / `visitor` 参数不再传播；这是静态路由问题，不要先改 CloudBase 数据 |
