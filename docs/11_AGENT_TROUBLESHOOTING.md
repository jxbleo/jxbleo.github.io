...
| Speaking Lab 选择音频后提示 `Upload information is incomplete.` | `app.getUploadMetadata()` 的临时上传字段位于 `metadata.data`，旧适配器错误读取了顶层字段，因此网页拿不到 URL/token/authorization | 部署最新版 `speakingLab`；上传投影必须先解包 `metadata.data`，同时返回 `file_id`、`cos_file_id`、`url`、`authorization`、`token` 和 `cloud_path`。不要记录或持久化临时上传凭据 |
