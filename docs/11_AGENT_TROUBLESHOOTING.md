...
JSON 编码：`Data[0]` 解析成数组后，单条记录仍可能是 JSON 字符串。逐条解码并
校验字段白名单；不要直接打印响应。分页不要假设所有集合的 `_id` 都是字符串。
本轮使用服务端 `_id` 排序、每页 100 条、有总页数上限的 offset 查询，重复键或
异常响应立即停止；连续两遍投影数据相同才记录稳定观察结果，不将其称为事务快照。

迁移函数的 `limit` 不一定限制扫描量：到期周补写读取全部 assignments，班级迁移
读取四个集合，STAR 迁移读取所有 achievement 并逐个查 ledger。清理核查只读取必要
元数据，不调用迁移、不查询答案和 grading keys、不把学生身份写入输出。零候选不代表
旧客户端已退役；非零候选也不授权自动修复。此轮确认历史到期周兼容仍有数据依赖。

## 2026-09-21 — 死代码清理不一定需要重新部署云函数

先构建清理前后的同一函数，再决定是否部署。此轮六个函数关闭 identifier minification
后产物逐字节相同：未引用函数已经被 esbuild 删除；submitAttempt 的正常压缩包差异
只是变量命名频率变化。已有线上版本与主线包的差异不能趁清理覆盖，应保留线上代码，
独立核查差异来源。下载线上 ZIP、配置指纹和 ACL 检查仍作为发布前证据保留。
本地 Manager 直接读取过期临时凭据可能返回 TokenFailure；官方 CLI 的只读列表调用
可刷新现有登录，再重试只读预检。不要输出认证文件内容或申请新的长期密钥。

## 2026-09-21 — 清理旧代码时的基线和测试陷阱

共享工作区可能停在旧 rebase，且同时包含已发布文件的未跟踪副本。先备份工作文件
和 rebase 元数据，在已核实的 origin/main 上建立独立 worktree；不能把相对旧 HEAD
的差异当作可删除内容。旧发布克隆也可能含主仓库没有的提交，删除前需要保存并验证
Git bundle。缓存版本号应验证格式或一致性，不应让回归测试锁定历史日期。只被测试
调用的 UI 函数需要核对当前产品约定；缺少调用者不等于产品功能已获准取消。
...
| Assign 选择班级时能看到学生，但 Teacher View 对应班级没有该学生 | Assign 使用当前学生名单，而旧矩阵只从 assignment/progress 反推行；转班后旧进度还可能保留原 `class_group`，完整 attempt 扫描超时又会让前端沿用旧缓存 | 发布同一提交的 `teacher.js` 和 `teacherAdmin`；矩阵应先建立当前 active roster 行，再 outer-join 进度，并以当前 profile 班级覆盖旧快照；summary 查询只投影必要 attempt 字段 |
...
| 手机 Student lookup 工具栏被拉高、Info/Progress 上方大块空白，Account 编辑拥挤 | viewport-height utility shell 使 lookup 的默认 auto 网格行拉伸；Account 外层 auto-fit 仍会在部分手机宽度分为两列 | lookup 行使用内容高度并顶部对齐；手机 Account 外层单列、输入独占整行。版本 `20260912-student-mobile-1` 同步更新 Teacher CSS/JS/config；检查 320–760px、滚动后 Edit/Cancel 和用量异步刷新时的草稿保留 |
...
| 历史 STAR migration apply 在 10 秒处报 `FUNCTIONS_TIME_LIMIT_EXCEEDED` | 这是旧发布期问题；迁移已完成，`migrateStarRewards` 生产入口于 2026-09-21 退役 | 不要再调用旧 action。先做只读 ledger/achievement 完整性审计；若未来发现新缺口，应新增一个有边界、经 owner 批准的修复流程，而不是恢复全表迁移入口 |
...
Listening is Dictation-only as of 2026-09-22. A stale client calling a retired
recording, scoring or preference action receives `ACTION_NOT_SUPPORTED`; deploy
matching cache-busted static assets and the updated function. Do not restore a
provider or change credentials to fix these retired actions. An old cloud
maintenance timer remains active until explicitly retired in CloudBase.

### CloudBase CLI 3.7### CloudBase CLI 3.7 code-only Argue rollout
