...
| 已登录 Speaking Lab 一直停在 `Loading your Discussions…`，且 `speakingLab` 云日志没有任何对应调用 | 页面通过 `getSession()` 后又并行运行两次 `callAuthenticatedFunction` 登录预检；浏览器 SDK 可能让两个预检都悬空，函数根本没有收到请求 | 发布最新版 `speaking-lab.js` 与缓存版本。页面只在启动时完成一次 `getSession()`，后续调用直接进入仍会在服务器验证 UID/角色的 `speakingLab`；刷新后确认 Discussion 状态收敛并有云日志，不要靠放宽函数或数据库权限绕过 |
