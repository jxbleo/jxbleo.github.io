...
After the 60-second response, show large red 3 → 2 → 1 in the same central digit slot as the opening countdown. Keep the faint circular track and tinted center. Recording fades out at the unchanged circle center over 160 ms; the central digit fades in over 180 ms after a 140 ms delay. Only the label-to-digit change animates; switching 3/2/1 does not restart the transition. A separate blinking red dot above the digit continues to indicate capture. The circle itself does not pulse during ending. Reduced Motion replaces the label immediately and keeps the red dot steady. Hide the duplicate external clock visually and from accessibility while retaining its space. The same MediaRecorder continues uninterrupted until 63 seconds; opening countdown remains unrecorded. Stop is still available during the warning.
...
Group Discussion shows a blinking red dot and Recording inside the central stop button, shared by the student and teacher hosts. The top capture indicator is removed during recording. The dot uses the IR 1.2-second full/dim cycle; Reduced Motion keeps it steady. During the final minute and ending cues, Recording sits above the large digits within the circle without overlap. The main stack retains its existing 112 px top padding and outer-wave clearance. The circle-to-clock gap is 28–40 px, with an 18 px gap on short screens; preserve the clock slot during the final minute.

## Discussion IR-style controls (2026-09-13)

Group Discussion ready uses the IR microphone and Tap to Start in the circle. A large mm:ss time below it opens the existing duration wheel; its glyph width is roughly 80% of the circle. The Upload button beneath the time uses the IR upload icon. Normal capture shows centred Recording with a blinking red dot inside and mm:ss below. At the final minute, hide the external time and show 60–01 inside beneath Recording; retain the reserved clock space to avoid shifting the circle. Opening/ending cues remain inside. Finished takes show a check in the circle with their duration and Submit below.
...
The question heading contains only question text, without year, Set metadata or a Q-number prefix, and stays visible and accessible during opening, recording and ending. The recording indicator replaces the stop square inside the circular button; there is no separate indicator row. The circle remains the stop action, with an accessible Recording — Tap to stop label. After the opening three seconds, a blinking red dot and steady `Recording` label show actual capture. The dot alternates full/dim opacity every 0.6 seconds; Reduced Motion keeps it steady. Hide the indicator immediately on Stop, completion or error, and during permission/countdown.

Both automatic completion and manual Stop show a centred thick green checkmark with `Tap to start over` directly beneath it inside the circle. The whole circle remains the restart button; there is no microphone or extra saved heading. The lower field reads `Your recording was successfully saved.`, centred in green, with no duration digits. Duration remains frozen and retained internally for submission. Keep explicit Submit and the existing recording replacement confirmation; this message does not mean the recording has already uploaded.
...
The IR native dialog now fills the viewport with no floating-card border, shadow, scrim or outer margin. Back at the top left returns to the Set; only the question appears above the recorder, without year, Set metadata, title duplication or a Q-number prefix. The IR content is a top-aligned continuous stack, without remaining-height centering. The circle uses min(80vw, 40svh, 360px). Question-to-circle, circle-to-clock and clock-to-footer gaps are 32/20/24 px; below 700 px height they become 24/16/20 px and the circle uses min(76vw, 36svh, 320px). Safe-area padding and vertical scrolling accommodate smaller viewports and longer questions. Hidden footer controls retain only their bottom slot, so capture does not move the circle. Microphone and opening digit scale with the circle. The larger clock remains below the circle. Keep the surrounding-control fade, stationary recording geometry, 280 ms theme transition and Reduced Motion support. IR capture is 60 + 3 seconds with three final cues; Discussion also ends with three seconds. No automatic submission is added.


## IR full-capture fade (2026-09-13)

The native IR dialog preserves its dimensions and recorder position while the card surface, border, shadow, Close and footer fade out over 680 ms. The question remains visible and accessible during opening, capture and ending; no Set label is rendered. Its backdrop and full-page surface become opaque deep charcoal, suppressing the underlying page throughout opening, recording and ending. The clock fades out for opening only and returns for the 60-second recording; the stop hint is removed from layout so Recording stays at the circle centre until it crossfades into the final 3/2/1. After all three final warning seconds, surroundings restore in 220 ms. Hidden surrounding controls are inert and excluded from accessibility navigation. Cancel, errors and early stop restore them too. Reduced Motion uses 100 ms transitions and retains existing non-pulsing countdowns.
...
### Dictation-only Listening surface (2026-09-22)

Listening Library cards open Dictation directly. The header displays a static
Dictation label, with no dropdown, mode switch or switch-confirmation modal.
Practice keeps Back, Dictation progress and the static label. Completion opens
as soon as Dictation is complete; it never waits for another track. Teacher
Listening keeps Details and Transcript & Units; there is no mode-preview tab.
Recording, microphone permissions, voice replay and pronunciation scores are
removed. Existing contextual BBC/IELTS links still open the same practice page.

The existing teal palette, glass surfaces, responsive spacing and accessible
playback/word controls remain. Historical non-Dictation learning time is labelled
Archived practice in calendar/report metadata; new practice links open Dictation.
