...
- BBC attempt 邮件主题只显示学生姓名、练习名称和该线程历史最高分；Vocabulary
  另外显示 `Quiz No. n` 或 `Practice No. n`，使每次提交在邮箱中独立可辨。所有主题
  都不带产品名称；邮件柱状图、逐次详情和纯文本回退都按最新一次在前排列，使第 4
  次位于第 3、2、1 次之前
- 邮件正文采用教师任务弹窗的静态同款视觉：姓名与练习名作为唯一顶部身份层级，
  紧接 Best / PASS / 可用时的 STAR 胶囊，再显示最新在前的历史柱状图和逐次错题
  卡片。学生答案、标准答案与 Explanation 使用红、绿、紫三个清晰区域。正文不显示
  产品品牌、Login ID、新提交摘要条或网页/卷子入口
...
- Vocabulary 每条事件立即到期并独立发送一封邮件。Quiz 与计时 Practice 的邮件主题
  分别带 `Quiz No. n` / `Practice No. n`，且不使用 SMTP reply/reference 头，避免邮箱
  把后续提交折叠成同一封会话。每封邮件仍重新投影截至该次提交的完整线程历史，
  因而第 2 次起天然包含并对比同一 assignment／自学 set 此前的 attempts。
