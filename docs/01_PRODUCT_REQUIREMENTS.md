...
- 通用语言批改的完成页必须恰好使用三个主卡片：`Language Review`、`Draft`、
  `Sentence Revision`，三个英文标题必须共用同一字号、字重、行高和字距。Sentence Revision
  大卡片第一行显示标题，右侧使用 44px 触控区的 `−` / `+` 调整本卡片内 AI 分析、历次反馈和
  Sample 参考修改的字号；调整采用有限档位并保存为浏览器显示偏好，不改变原句、学生答案、标题或
  ...
  `CORRECT`、`REVISED` 或 `NEEDS REVISION` 字段：原句无需修改或订正通过使用红色圆圈勾；空白、
  尚未通过或未改变上次未通过答案使用黑色圆圈叉；学生新输入或改变了尚未 Submit 的订正文本使用
  黑色圆圈问号。无需修改的句子只
...
- Sentence Revision 行首编号采用 BBC classroom worksheet 填空题的裸序号风格：小号粗体，
  不使用边框、圆底或胶囊背景。数字导航胶囊下方使用卡片状态图标的缩小版：正确为红色圆圈勾，
  新输入且尚待检查为黑色圆圈问号，空白或最后一次检查未通过为黑色圆圈叉。输入、清空或修改文本时，
  胶囊与卡片右上角必须同步更新；可访问名称同时说明正确、等待检查或错误。
...
- 句子一旦通过 Check，卡片默认翻到学生已订正句子的一面，以同句索引色高亮正文，并在右上角显示
  红色圆圈勾；此状态不再显示禁用输入框或句末图标。学生主动翻面后才重新看到原句与语法分析。
...
- 参考句可以在分析正面按需展开，但翻到输入背面后必须与分析和参考句一起隐藏，不能与输入框同时出现。输入框提示为
  `Rewrite this sentence in your own words. Do not copy the sample.`。改写采用
