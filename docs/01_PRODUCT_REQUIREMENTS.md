...
- 教师主页只用轻量状态读取显示铃铛未读线程数。通知摘要每批 5 个线程，按最新在上；
  如果未读超过一批，浏览器自动继续分页直到覆盖全部未读线程，然后停止在已读历史
  边界，只有 `Load 5 more` 才读取更早内容。所有已读取的未读线程会在当前标签页内
  自上而下静默预取逐题答案与解释；后台队列不显示旋转加载状态，也不把私有详情写入
  持久浏览器缓存
