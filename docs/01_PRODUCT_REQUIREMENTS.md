...
- AI Tutor 页面只保留一个顶部工具栏；工具栏不再显示品牌图标、`AI Tutor` 或 `Writing Studio`
  字样。最左侧返回箭头仍返回 Dashboard，但必须先显示自定义确认弹窗，并明确说明已保存内容不会
  丢失、正在进行的 OCR 或 AI 批改会继续在后台运行。
- 作品库按钮占用原品牌区域。手机、iPad 与桌面使用同一可收起的左侧抽屉模型，初始均为收起；
...
- 通用语言批改的完成页必须恰好使用三个主卡片：`Language Review`、`Draft`、
  `Sentence Revision`。Sentence Revision 卡顶部使用可横向滑动的数字胶囊定位对应句子，胶囊
  使用统一的 `1px` 实线边框；已完成的胶囊在数字下方显示小勾，待完成或复查可显示小点，但下方
  不显示填写进度或操作提示。全部句子始终以列表呈现，不提供逐句模式、布局切换或两种布局图标。
  ...
  区块。无需修改的句子只
  显示原句及其后的矩阵同款圆形 SVG 勾，不得显示语法框或“你的改写”；圆底继承该句子的索引色。
- Sentence Revision 行首编号采用 BBC classroom worksheet 填空题的裸序号风格：小号粗体、
  baseline 对齐，不使用边框、圆底或胶囊背景。数字导航胶囊中，已完成继续在下方显示小勾；
  尚未完成或检查后仍需订正的句子在下方显示小问号，不再显示圆点。
- 句子一旦通过 Check，卡片默认翻到学生已订正句子的一面，以同句索引色高亮正文并在句末显示
  矩阵同款圆形 SVG 勾；此状态不再显示禁用输入框。学生主动翻面后才重新看到原句与语法分析。
- Sentence Revision 中无需修改的原句也必须放进与其他句子相同的有边框卡片表面，不能退化成
  无框文本行；它只有一个静态面，不伪造可翻转内容。编号与正文使用固定编号列和正文列，
  长句换到第二行时必须与首行第一个字母对齐，而不是与编号对齐。
- 删除卡片内“我记住了，开始改写”“返回查看分析”等翻面按钮。除 Sample 与输入框等独立控件外，
  单击卡片任意位置即可翻面；同一覆盖式原生按钮同时提供键盘焦点和无障碍名称。
- “查看参考句”统一改名为 `Sample`。按钮放在分析段落之后的右侧，展开的参考句紧随其后；
  Sample 是分析面的次要操作，不能与整卡翻面点击冲突。
- 双面卡片必须支持按钮、键盘和屏幕阅读器切换。正常动画可以表达翻面关系；启用
  `prefers-reduced-motion` 时必须改为无旋转的即时状态切换或短淡化，同时仍只暴露当前一面。
- Sentence Revision 底部不得显示“未完成的句子……”等说明，只保留一个文本严格为 `Check`
  的提交按钮；第二轮检查仍使用同一按钮名称。
- 返回箭头使用红色警示语义；确认弹窗复用练习页的紧凑 Apple 风格，提供绿色 `Cancel` 与红色
  `Leave`。句子数字导航向上滚动到视口顶部后必须取代主工具栏吸顶，而不是停留在工具栏下方。
- 原文中的每句话、对应数字胶囊和句子批改中的原句必须共享同一组循环配色。句子高亮不使用文字
  下边线，并避开全站作为主操作及状态语义使用的绿色；八色循环固定使用蓝、橙、紫、玫红、靛蓝、
  珊瑚、金黄和深粉。点击原文句子或数字胶囊都必须激活相同句子并滚动到对应批改项。所有卡片和
  动态英文内容必须在手机宽度内收缩；只有数字胶囊行允许横向滚动。
- `Draft` 必须按照学生确认原稿中的空格和段落换行连续排版，不能把每个句子渲染成独立段落。
  同一段中的不同句子仍使用各自的高亮颜色，并可分别点击定位；句子边界的空格和换行必须留在
  高亮元素之外，确保真实段落结构不受分句结果影响。
- 参考句可以在分析正面按需展开，但翻到输入背面后必须与分析和参考句一起隐藏，不能与输入框同时出现。改写采用
