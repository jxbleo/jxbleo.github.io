...
- `Scan Revisions` 是可编辑 `Sentence Revision` 内的辅助入口，不是新的作文或提交类型。学生拍照或选择
  ...
  Review Scan 页面不显示标题、说明、缺失句子汇总或匹配状态文案，只保留句子卡片和底部返回/导入操作。
  卡片内不显示手写编号或扫描字段标签；识别置信度仅用极小的高 `✓` / 中 `!` / 低 `?` 符号显示，并提供可访问说明。
  学生按下导入即是明确采用当前卡片中已核对的扫描文字；它覆盖对应未完成句子的现有草稿。返回而不导入则不改动原草稿。
...
- 每次 `Submit` 的逐句 AI 点评必须按句子保留历史，不得用新点评覆盖上一轮。分析面先显示初始语言建议，其后按时间顺序显示
  `第 1 次点评`、`第 2 次点评`等；每轮点评上方使用一条细分割线。旧作文只有一份现存点评时，将其作为可恢复的第 1 次快照，下一次提交从第 2 次继续。
