...
- 只有在所有必改句均已通过且至少存在一条已保存的学生订正句时，Draft 卡片顶部才显示
  ...
  点击 Draft 的任一句打开 Apple 风格 `AI Feedback` 弹窗：原本正确的句子明确说明无需修改，原始
  必改句显示初始分析及已保存的历次反馈，反馈之间只使用细分割线且不显示轮次名称。Revised 句子
  不可点击。初稿本来没有必改句时不显示多余的 Revised；全文重组只使用现有持久化结果，不得新增 AI 调用。
