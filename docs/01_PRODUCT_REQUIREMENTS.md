...
- `Draft` 必须按照学生确认原稿中的空格和段落换行连续排版，不能把每个句子渲染成独立段落。
  原始 Draft 使用暖象牙色纸张、极轻纸纤维和深灰正文，不使用八色填充；尚未通过订正的原句使用
  克制的暗红色细波浪线。学生只输入但未 Submit、Submit 后未通过或仍为空白时都保留波浪线；某句
  最新订正结果被接受后立即取消该原句波浪线。句子边界的空格和换行必须留在可点击句子元素之外，
  确保真实段落结构不受分句结果影响。
- 只有在所有必改句均已通过且至少存在一条已保存的学生订正句时，Draft 卡片顶部才显示
  ...
  不显示波浪线。切回 `Draft` 始终显示未替换的确认原稿，并只标出仍未通过的订正项。初稿本来没有必改句时
  不显示多余的 Revised；全文重组只使用现有持久化结果，不得新增 AI 调用。
