...
BBC 习题页的自定义播放器显示由当前课程同一 MP3 解码得到的真实音频波形。学生可
点击或拖动波形定位播放位置，并用 1×、2×、4×、8×、16×、32×、64× 放大完整
时间轴后横向查看，也可用键盘左右方向键每次移动 5 秒。播放与回退按钮和缩放控件
位于波形上方同一行，使波形占满播放器宽度；波形下方只显示当前时间和总时长，不
显示学生姓名。缩放只改变时间轴的视觉宽度，不改变音频时长或内容；播放头、已播放
区域和点击位置必须始终与浏览器实际音频时间一致。波形生成失败时保留可定位的时间
轴和普通播放控制，不得阻断听力练习。
...
单元首次进入时自动播放。默认 `Tab` 重播、`Space` 前进词位、`Enter` 检查；
学生可从 Unit 栏最右侧的 `Keys` 设置在这三枚安全按键之间重新分配操作，保存后
在当前电脑立即生效并持久保留。练习卡不再常驻显示快捷键说明，底部也不显示
初始操作提示；反馈栏仅在检查、报错或其他真实状态需要反馈时出现，`Check` 按钮
只显示动作名称。

材料标题卡的四个概览项只显示 `Completed`、`Independent`、`With answer` 和
`Replays` 栏目名称，不显示四个动态数字。若材料绑定理解练习，入口统一显示
`Comprehension Practice`；点击后必须先在确认层选择继续，才可离开精听页面。
...
正确词位锁定，错误词位标红并聚焦第一个错误位置。学生无需先完成检查即可直接
`Show Answer`；答案以阻塞输入的模态层显示，默认 5 秒后自动关闭，底部进度条在
同一时长内走完。答案层打开后的任意点击会中断进度并取消自动关闭，之后必须由
学生明确点击 `Close` 才能返回词位输入。自主完成与查看答案完成分别统计。永久最高完成记录单调不降，`Clear & Start Again`
...
- AI Tutor 页面只保留一个顶部工具栏；工具栏不再显示品牌图标、`AI Tutor`、`Writing Studio`、
  ...
  标题尽量占满返回箭头与右侧操作之间的宽度，并在接近两侧按钮边缘时渐隐；三横线侧边栏按钮
  固定在工具栏最右端，工具栏不再显示标题铅笔。侧栏顶栏在加号同行提供一个铅笔作为标题编辑
  模式开关：激活后所有作文标题轻微错相抖动，点击某个标题才打开该作文的独立标题编辑弹窗；
  再次点击铅笔、关闭侧栏或点击加号会退出编辑模式。Reduced Motion 用静态绿色提示代替抖动。
  OCR 确认阶段仍可在右侧临时显示
  `Show image` / `Hide image` 上下文操作。
- 返回箭头先关闭已展开的侧栏；在具体作文中直接返回 Writing Home；新作文表单已展开时
  无损收起表单。所有 Writing 内部层级返回都不显示确认，只有位于 Writing Home 基层、即将离开
  Writing 功能返回 Student Dashboard 时才显示离开确认。
- 三横线控制同一个简洁 Writing sidebar。手机初始收起并以遮罩抽屉显示；iPad/桌面等宽度达到
  `820px` 时初始自动展开为停靠侧栏，但再次点击仍可隐藏。侧栏只保留顶栏加号、编辑模式铅笔和
  作文标题，不再重复 Home；
...
- 顶部工具栏中间在打开作文时显示该篇作文的 AI 或学生标题。标题过长时在可用宽度内往返横向
  滚动并在两端停顿，在接近 Back 与三横线按钮时平滑渐隐；Reduced Motion 下改为单行省略。
  标题编辑只从侧栏顶栏的单个铅笔进入：激活编辑模式后点击目标标题，再由 Apple 风格独立弹窗
  提供正常字号输入、Cancel 和 Save；保存继续调用受所有权保护的 `updateCompositionTitle`，且
  不改变作文活动排序。未打开作文时显示绿色 `Start new Writing`。
- 学生可直接输入，或上传最多八张作文照片。OCR 必须保留原始错误并标出不确定处；学生确认
  Confirmed Manuscript 后才可评估。确认后删除作文图片，只长期保存文字。通用语言批改的逐句卡片必须由
  服务器先确定边界和稳定 ID；成对引号内部的标点、引号后的对话标签或小写连接成分不得被错误拆成
  两张卡片，也不得跨段落自动合并。
...
- AI Writing Tutor 主页是行动优先的写作工作台，不是长期功能介绍页。删除时段问候和
  ...
  两个纯文字模式按钮；副标题分别固定为 `Grammar & Usage` 与 `Ideas & Structure`；其下把所有未完成 Composition 显示为可横向滑动的 Library 风格任务胶囊，
...
- 点击主页 `Polishing` 或 `Brainstorming` 不进入新的输入页面，而是在两个按钮下方原位展开表单；展开采用克制、可中断且尊重
  ...
  不得保留内容缩短前的底部滚动位置。底部主按钮按输入方式显示 `Submit` 或 `Scan`，只有点击底部 `Scan` 才上传并开始识别。底部操作使用明确的非对称层级：右侧绿色 `Submit` / `Scan` 约占 70% 宽度；左侧红色 `Discard` 约占 30%，保留完整点击区域和淡边框，但移除填充底色与阴影，避免与主操作竞争。空白稿直接删除并返回；存在学生输入时先显示永久删除确认弹窗。服务器仅允许删除 revision 1、尚未进入上传/OCR/评估/订正且没有 Library 题目绑定的 `draft`；不满足条件时必须拒绝，因此任何已开始处理或已提交作品仍不可删除。
- OCR 确认页正文编辑器下方右侧实时显示 `N words`（单数 `1 word`）；仅统计正文，采用 Draft/Revised 相同规则。
  输入、粘贴、删除、Use first line 和 Undo 均即时更新；标题与题目不计入，也不增加 AI 调用。
  题目 OCR 不显示正文词数。
- OCR 确认页删除 `OCR Review` 标题，并使用与 Draft 相同的暖黄色纸张材质。原图开关位于页面工具栏
  右侧，不再占用纸张卡片空间；关闭时显示 `Show image`，打开时原位显示 `Hide image`。正文上方保留
  小型 `Title (Optional)` 输入框、一个可逆的 `Use first line` 按钮、紧接其后的正文编辑器，以及底部
  唯一一个水平居中的 `Confirm`。`Use first line` 把首个非空行移动到标题而不是复制，防止标题继续进入
  逐句批改；成功后同一个按钮原位变成 `Undo`，再次点击恢复原标题、正文和存疑标记，不得同时显示两个
  操作按钮。超过 80 个字符的首行不得静默截断，学生可改为手动填写。
  ...
  删除。原图默认隐藏；工具栏按钮在电脑/iPad打开左右对照，在手机把图片放到编辑器上方，再次点击隐藏。
...
- 通用语言批改在订正进行中使用三个主卡片：`Language Review`、`Draft` 和逐句订正卡片。
  第三张卡片不再显示 `Sentence Revision` 标题；其第一行左侧是带调色盘 SVG 的 44px 皮肤按钮，
  右侧是 44px 触控区的 `−` / `+`，用于调整本卡片内 AI 分析和历次反馈的字号。绿色和彩色皮肤
  都必须保留整张卡片的顶部圆角；标题控制行的背景不得覆盖或方化圆角。皮肤默认采用与 Language Review 和早期
  BBC 页面呼应的统一绿色主题；点击后切换到现有八色循环主题，再次点击返回绿色主题。皮肤与字号均为
  浏览器本地显示偏好，不改变作文、句子、反馈或提交数据；皮肤切换必须原位完成，不重置当前句、输入焦点
  或滚动位置。所有必改句通过且 `Revised` 可用后，完成页只保留 `Language Review` 和带
  `Draft / Revised` 切换的全文卡片，整个逐句订正卡片不再显示。控制行随页面正常滚走，不吸顶。
  第二行使用占满卡片可用宽度的横向数字导航定位对应句子；每个胶囊必须是等宽等高的正圆，使用统一
  `1px` 实线边框，首尾保留安全区内边距且不得在到达屏幕边缘前被裁成半圆。只有该数字导航滚到视口
  顶部后吸顶并取代主工具栏。全部句子始终以
  ...
  区块。每张卡片左上角显示不占正文宽度的裸序号，右上角显示圆圈线框状态图标，错误句另提供举手 Argue 按钮，不显示
  ...
  逐句订正卡片不显示 `Sentence Revision` 标题或“亲自重写后再按 Submit”等说明；底部操作栏只在 `Submit` 左侧显示一个带可访问名称的小相机图标按钮，不显示 `Scan Revisions` 文字。
...
- 顶部工具栏不显示逐句订正百分比；订正完成状态继续由数字胶囊、卡片状态与 Draft/Revised 结果
  表达。Check 完成后不在工具栏下方显示“统一检查完成……”说明。
...
- Sentence Revision 中无需修改的原句也必须放进与其他句子相同的有边框卡片表面，不能退化成
  ...
  正文始终使用卡片完整宽度，长句换行与首行第一个字母对齐。点击或用键盘激活该正确句卡片时，
  卡片只给出一次轻微左右抖动，不翻面、不出现输入框；Reduced Motion 下改为短暂静态色彩反馈。
...
- 彩色皮肤下，数字胶囊和 Sentence Revision 卡片共享固定的八色循环：蓝、橙、紫、玫红、靛蓝、
  珊瑚、金黄和深粉；默认绿色皮肤下，它们统一使用绿色及浅绿色表面。完成后的 `Revised` 全文继续
  使用八色句子映射，以便与原稿句子建立稳定对应。点击 Draft/Revised 句子或数字导航都必须激活相同
  句子并滚动到对应批改项。所有卡片和动态英文
...
- 第二张全文卡片的右下角始终以低强调英文显示当前版本词数。订正未完成、只有 Draft 时显示确认原稿的
...
Formal browser recording follows Ready, a full-screen opening cue, Recording, Review, then Uploading. After microphone permission, English TTS announces the start, a visible and audible five-second countdown runs, and the live waveform follows microphone level. During recording, one circular progress ring advances around the digital remaining-time value. At one minute remaining before the selected target, the progress ring resets to full and drains over the remaining 60 seconds while the displayed countdown stays continuous. The reminder sounds three times, each lasting 0.36 seconds, with 0.19-second gaps and unchanged pitch and volume. This three-sound reminder runs once per recording. The ring retains its final-minute warning colour. Recording has one Finish action and no Pause. At the selected target, a final five-second warning turns the ring and digit red with a gentle once-per-second opacity pulse, and sounds one beep at each displayed second (the last is more prominent). Reduced Motion retains red without pulsing. The recorder stops automatically at target plus five seconds (8:05 for an eight-minute Discussion). Review offers Play, Replace, or `Upload & analyse`. The local recording remains only in
...
Intensive Listening, and Speaking as three independent full-width entrances
below the welcome hero in that order. Each card uses a separate pale glass
surface with a blue, teal, or orange category wash, a visible English title,
purpose copy, and a circular `写`, `听`, or `说` identifier. Phone layouts wrap
the purpose copy inside the card instead of horizontally scrolling it. The
Speaking entrance is explicitly labelled `SPEAKING · HKDSE PAPER 4`, describes
Group Interaction, and repeats `HKDSE Paper 4 Speaking` in its confirmation.
An ordinary click or tap asks for confirmation before opening the existing
destination; modified desktop clicks retain the browser's normal new-tab
behavior.
...
`Start Response` control or intermediate page. The Individual Response recorder uses the same circular remaining-time display, advances from `01:05` to `00:00`, and changes to a red warning for the final five seconds;
...
### Argue 每日未处理提醒（2026-09-06）

学生 Argue 提交当天只发送原通知。从提交次日开始，按北京时间每天
11:30 对每条仍未处理的请求发送一封“提醒处理”邮件，继续保留原题内容、
三选项处理入口和选填 Teachers’ Note，直到处理完成。已有未处理学生请求也
适用；教师主动纠错不发送提醒。服务器按分钟调度，11:30 起顺序投递，临时
延迟可在当天补执行，不补发往日提醒。

邮件入口与教师网页共用 `answer_disputes` 的处理结果；任一入口成功处理完成
后停止提醒。如果提交决定后重算失败、状态仍是 pending，次日继续提醒，
方便从原入口继续处理。发信前再次检查，取消作业或删除学生
的请求跳过。收件人继续使用个人中心启用邮箱；全部关闭时跳过当天邮件，
重新启用后从后续日期正常提醒，不补发已跳过邮件。

### 作文逐句 Argue（2026-09-12）

- 错误句卡片右上角在状态图标旁增加举手按钮。初次判错和改写后仍被判错均可申诉；发送的是最近一次已检查的文本，尚未提交的改写不会发送。弹窗支持可选理由。
- 同句当前待处理请求复用；教师 Reject 后可不限次数重新申诉，每次保留独立记录。Reject 不把句子算对。
