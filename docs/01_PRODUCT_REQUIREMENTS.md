...
- 点击右上角姓名打开 Personal Center；卡片不显示右上角叉号，唯一的 `Close`
  胶囊独立位于卡片外部正下方
- `To Do List` 默认弹窗不显示 To Do / Upcoming / Finished 三个顶部统计胶囊；首栏命名为
  `THIS WEEK`，底部 `FINISHED` 默认收起，学生点击后才展开
- 首页大卡片固定显示 `OVERDUE`、`THIS WEEK`、`UPCOMING` 三条摘要；每条按
  该组已完成数除以总任务数显示真实进度条和百分比。点击任一摘要会打开仅含
  对应分组任务的 Assignments 弹窗，首页本身不直接展开任务行
- `To Do List` 弹窗右上角放置纯对话气泡图标；点击查看全部历史老师回复，
  图标内部不带勾，红点只提示未读回复。回复弹窗没有关闭叉号或底部 Close，
  左上角 `Back` 标记当前回复已读并返回同一个 Assignments 弹窗
...
- 从铃铛右边的笔记本图标打开独立 My Words 生词本弹窗；关闭后再次打开时
  恢复到上次的滚动位置
- My Words 弹窗在桌面与手机上都保持紧凑并上下居中，主体不会占满手机屏幕；
  `Close` 按钮独立位于弹窗卡片下方
- 右上角使用同一视觉样式的搜索与加号图标；加号只展开单行词汇输入框，
  按 Enter 即可保存，不要求填写上下文
...
学生端周进度按 `due_at` 计算：首页固定显示 `OVERDUE`、`THIS WEEK` 和
`UPCOMING`，分别以组内完成数/总任务数显示进度条和百分比。点击任一分组
打开对应任务列表；下周任务不计入红色数字。红色数字只计算逾期、
...
只修改该学生对应记录。底部左侧为红色 Cancel open assignments、右侧为 Save
changes；点击取消作业后必须在第二层独立弹窗再次确认。
