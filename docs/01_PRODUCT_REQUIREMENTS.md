...
替代完整覆盖。服务先用完整转录生成全局讨论、考生总评及三个评分维度，再按每位
考生最多八个目标发言分块生成逐轮 CS/IO 深度点评；分块包含相邻互动上下文，最终
由服务端按稳定 `turn_id` 合并成原有完整报告。学生和教师看到的报告字段、逐轮
点评数量及评分范围不减少。

每个已完成分块必须先持久化；中断后从第一个未完成分块继续，不重复转写、声纹
匹配或已完成的模型分块。Schema/无效响应每个分块最多一次修复重试，超时、限流、
传输错误和供应商 5xx 每个分块最多两次重试。AI Usage 应显示安全的错误分类和
HTTP 状态，不得显示提示词、转录、模型正文或供应商请求 ID。
...
无需经过 Focus Session 或开始倒计时；浏览器允许时自动播放，若阻止自动播放，
学生可使用练习卡上的 Replay 按钮。后续单元首次进入时自动播放；完成后的临时重练
也直接返回练习卡。默认 `Tab` 重播、`Space` 前进词位、`Enter` 检查；
...
2026-09-21 经所有者确认的一次性历史修复：仅对审核清单中的 37 条启用账号、
`due_at` 为空的待完成作业，按上海时区的实际 `created_at` 推算截止周；周一至
周五归当周，周六/周日归下一周，均补为对应周日 23:59:59。不对已删除账号或
来源不明记录推算，不改原始创建/布置时间。此规则只用于本批回填，不改变新作业
必须显式选择截止周的要求，也不替换其他历史记录现有的兼容读取规则。
