...
function individualResponseAnalysisPrompt() {
  return [
    ...
    "DOMAIN FEEDBACK: each IO and VL domain must contain strengths and weaknesses arrays. Aim for 2–3 distinct evidence-grounded points in each array when the answer supports them; never invent extra points to fill a quota. A strength identifies the exact wording/idea, puts its supporting IDs only in evidence_segment_ids, and explains specifically how it helped this dimension. A weakness identifies a concrete gap or reliable recurring language issue, puts its supporting IDs only in evidence_segment_ids, explains the effect on communication and gives an actionable improvement_zh plus a short usable example_en. Explain why the score fits the original answer in commentary_zh. Avoid generic praise, repeated points across domains and lists of unsupported grammar corrections. For omitted reasoning, cite the nearest relevant idea and describe the missing link without inventing words the student said. With insufficient usable evidence, leave the appropriate array empty and explicitly explain that limitation in commentary_zh.",
    "IMPROVEMENT-FIRST COACHING: the weaknesses array powers a warm How to improve panel, not a list of faults. Write point_zh as an achievable action heading (for example, explain how the example supports your reason), never a label judging the learner. In explanation_zh briefly identify the evidenced gap and its effect; make improvement_zh the main teaching content. Give concrete, tailored steps: what to add/change, which idea to develop or reliable phrase to improve (without numbered locations), and how the change helps this IO or VL dimension. Use two or three short steps when useful, not a mandatory quota or generic advice such as use better vocabulary. Follow with example_en demonstrating that exact improvement in natural spoken English while preserving the student's established meaning. IO should teach a specific link, example, sequence or qualification; VL should teach a specific reliable wording/grammar/collocation choice and why it works. Hypothetical additions must be clearly marked and cannot invent personal facts. Prioritise useful changes for the next attempt. Keep the score based on the original answer and retain all evidence and ASR safeguards. Strengths must explain what the student can keep doing, with exact supported evidence, not empty reassurance.",
    "STUDENT-FACING VOICE: write all Chinese feedback directly to the learner using 你 / 你的, including summary_zh, commentary_zh, point_zh, explanation_zh, improvement_zh, student_viewpoint_zh, student_idea_zh and thinking_prompt_zh. Never refer to the recipient as 該學生, 该学生, 學生, 考生, the student, the candidate, or he/she. This restriction concerns references to the recipient, not an actual discussion topic involving students. Use a warm, specific coaching voice; do not force 你 into every sentence or invent personal details. English example_en and response_en remain natural proposed speech in the learner's voice, using I when appropriate, not second-person feedback to the learner.",
    "INTERNAL EVIDENCE BOUNDARY: supplied segment IDs belong ONLY in structured evidence_segment_ids arrays. Never put seg_0001, segment IDs, timestamps, paragraph/line/turn numbers, JSON keys, or technical evidence citations in any prose field, including English examples. Refer naturally to the learner's actual wording or idea with a short reliable quote or paraphrase, without locating it by number or time. For example, write 你提到照顧植物能培養責任感，這讓你的理由更具體。 Do not write 該學生在 seg_0001 表達了觀點 or 你在第 3 段提到…。 Keep valid internal evidence arrays intact; hiding technical references must not remove grounding or invent quotes.",
...
function individualResponseUserPrompt({ questionText, context, segments, schemaVersion } = {}) {
  ...
  return [
    ...
    JSON.stringify({
      summary_zh: "Traditional Chinese feedback addressed directly as 你, without technical citations",
      domains: Object.fromEntries(["ideas_organisation", "vocabulary_language_patterns"].map((key) => [key, {
        ...
        strengths: [{ point_zh: "Specific strength", explanation_zh: "Address 你; quote or paraphrase wording/idea and explain its effect, without segment/location labels", evidence_segment_ids: ["valid supplied segment ID"] }],
        weaknesses: [{ point_zh: "Achievable action heading", explanation_zh: "Brief evidenced gap and impact", evidence_segment_ids: ["valid supplied segment ID"], improvement_zh: "Steps tailored to 你, which idea/phrase to develop, and why they help; no numbered locations", example_en: "Short usable improved example" }],
      }])),
    ...
    "Before returning, check EVERY prose field: address the recipient directly as 你 / 你的 rather than 該學生 / 该学生 / the student; remove all segment IDs (such as seg_0001), timestamps and numbered paragraph/line/turn references. Use the actual idea or a reliable short quote instead. Keep supplied IDs ONLY in evidence_segment_ids arrays, and keep English examples as proposed speech in the learner's own voice. Do not alter scores or fabricate evidence to satisfy these wording rules.",
